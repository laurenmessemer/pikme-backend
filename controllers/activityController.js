...
exports.getTopVoters = async (req, res) => {
    try {
      ...
      competitions.forEach((comp) => {
        if (comp.votes_user1) {
          voteMap[comp.user1_id] = (voteMap[comp.user1_id] || 0) + comp.votes_user1;
        }
        if (comp.user2_id && comp.votes_user2) {
          voteMap[comp.user2_id] = (voteMap[comp.user2_id] || 0) + comp.votes_user2;
        }
      });
      ...
      const me = sorted.find((user) => String(user.id) === String(userId)) || { username: "Me", count: 0 };
  
      res.json({
        me,
        topVoters: sorted.slice(0, 5),
      });
    } catch (err) {
